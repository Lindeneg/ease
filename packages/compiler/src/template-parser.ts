...
export type TemplateNode = TextNode | InterpolationNode | ElementNode | SlotNode;
...
export function parseTemplate(source: string): ParseResult {
    ...
    if (hasErrors) return failure({output: nodes, diagnostics});
...
class Parser {
    ...
    private parseClosingTag(expectedTag: string, elStart: number, parentTag: string | null): void {
