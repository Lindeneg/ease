...
const BLOCK_OPEN_RE = /^<(template|script|style)(\s[^>]*)?\s*>/gm;
...
/** A raw top-level block  before any parsing. */
...
/** The result of splitting into top-level blocks. */
export interface SplitResult {
    /** `<template>` block, or null if absent */
    template: RawBlock | null;
    /** `<script server>` block, or null if absent */
    serverScript: RawBlock | null;
    /** `<script client>` block, or null if absent */
